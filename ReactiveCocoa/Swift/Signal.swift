		...
		if let token = token {
			return ActionDisposable { [weak self] in
				if let strongSelf = self {
					strongSelf.state.modify {
						$0?.observers.remove(using: token)
						if $0?.observers.isEmpty ?? false {
							$0!.retainedSignal = nil
						}
					}
				}
			}
	...
	public func zip<U>(with other: Signal<U, Error>) -> Signal<(Value, U), Error> {
		return Signal { observer in
			...
			let flush = {
				...
				state.modify {
					guard !$0.values.left.isEmpty && !$0.values.right.isEmpty else {
						isFinished = $0.isFinished
						return
					}

					tuple = ($0.values.left.removeFirst(), $0.values.right.removeFirst())
					isFinished = $0.isFinished
				}
